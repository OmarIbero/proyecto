...
saveTrigger.addEventListener("click", () => {
    ...
    .then(res => {
        console.log(res)
        if (res.data === "saved"){
            showNotif(notifstates.saved,"/makefs/views/iconos/book.png");
            setButtonState(true);
        }else{
            showNotif(notifstates.removed,"/makefs/views/iconos/remove-book.png");
            setButtonState(false);
        }
    }).catch(err => {
        showNotif(notifstates.error, "/makefs/views/iconos/error.png");
        setButtonState(isSaved);
     })
})